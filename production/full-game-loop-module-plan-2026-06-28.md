...
### M13 — Sovereign / Hegemony Loop（君主与争霸循环）

**职责**：忠臣继业或自立成功后，进入君主级国策、任命、争霸和统一路径。

| 字段 | 内容 |
|---|---|
| 玩家体验 | 成为君主后不是换成全知面板，而是拥有更高权限、更大责任和更复杂的信息盲区 |
| 权威状态 | SovereignState、PolicyState、MandateLegitimacy、VassalNetwork、WarTheaterPortfolio |
| 输入 | 继承、自立、称王、国策、任命、征伐、外交盟约 |
| 输出 | 势力级战略、统一进度、派系稳定、终局条件 |
| 依赖 | M09/M10/M11/M12 |
| 风险 | 当前无 GDD；若直接实现会变成另一个游戏 |
| 建议 epic | 暂不建实现 epic；先建 `GDD_017-sovereign-hegemony.md` + ADR |
| 测试证据 | 君主玩法仍遵守 GDD_007 信息分离；国策影响多系统但可解释 |

### M14 — Victory / Ending Loop（统一与终局循环）

**职责**：定义什么叫通关、不同路线如何结局、失败如何留下结局评价。

| 字段 | 内容 |
|---|---|
| 玩家体验 | 玩家能从太守一路走到乱世终局，并看到自己选择造成的历史版本 |
| 权威状态 | VictoryCondition、EndingState、LegacyRecord、HistorySummary |
| 输入 | 统一进度、势力存续、玩家路线、关系结局、历史分叉 |
| 输出 | 统一胜利、多结局、战后评价、可继续或新周目钩子 |
| 依赖 | M10/M13 |
| 风险 | 无 GDD；结局条件如果只按城池数量，会削弱人生与关系支柱 |
| 建议 epic | 暂不建实现 epic；先建 `GDD_018-victory-ending.md` + ADR |
| 测试证据 | 多路线结局条件互斥且可达；关系/城市/历史分叉进入结局摘要 |
...
4. M06 Tactical Battle
5. M08 Enemy AI 战术层
6. M15 Meta/Battle UX

完成标准：玩家能通过至少两条非按钮条件链以弱胜强；敌方会根据有限认知反应；非战斗系统影响战役结果。
...
| epic-026 | Sovereign / Hegemony | Feature | GDD_017 + ADR | 不可直接实现 |
| epic-027 | Victory / Ending | Feature | GDD_018 + ADR | 不可直接实现 |
