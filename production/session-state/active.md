...
> - ⏳ **D 三维细化**（#5）— GeneralOf 现恒 0.6/0.7/0.8，名将差异全靠 tags/档。待：Command/Valor/Guile 从 Prowess/Strategy 档 + tags 确定性派生，拉开差异。
> - ⏳ **E 守备接城池/经济**（#6）— DefenseOf 现恒 600/1.2。待：守军/工事随城市经济/驻将/纪元。
> - ⏳ **F 招揽状态机**（#2，大）— PoolAt 现只候选池+难度标签、无反全知门。待：KnowledgeState（未闻→听闻→定位→接触→招揽中→加入/婉拒/结怨/投敌）+ 发现渠道 + offer/attempt/cooldown + 反全知门（未知不可见）+ 入僚属。
> - ⏳ **G GeneralState 运行时人生**（#1/#4，最大）— 待：持久化 per-general 态（主君/位置/官职/伤病/疲劳/忠诚/名声/任务/被俘/记忆）+ 存档迁移 + 喂关系/AI。
> - ⏳ **H 数据外部化**（#5 维护）— GeneralDossiers 巨型硬编码 → 外部数据 + 加载器承接校验。
