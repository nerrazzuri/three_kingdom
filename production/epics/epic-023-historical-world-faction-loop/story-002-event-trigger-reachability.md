# Story 002: 历史事件按时间窗触发（够不着前置短路恒成立）
...
见 `tests/unit/ThreeKingdom.Domain.Tests/World/CampaignHistoryTriggerTests.cs`（详见 Completion Notes）。
...
**Required evidence**:
- `tests/unit/ThreeKingdom.Domain.Tests/World/CampaignHistoryTriggerTests.cs` — 必须存在且全绿
...
**Test Evidence**: `tests/unit/ThreeKingdom.Domain.Tests/World/CampaignHistoryTriggerTests.cs` — 4 tests, 全绿（797/797）
