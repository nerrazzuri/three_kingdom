...
> **军纪项裁定（2026-07-04）**：离间的"军纪罚"作用于守方的**有效稳定度**——在战斗接缝并入守方防御士气/工事系数（`DefenderDisciplineDelta` 与士气 delta 同路削弱守方战力）。这是**正式模型**（军纪=压力下维持战力的能力，其外显即有效士气/防御），**非权宜**；区域引擎不另设独立军纪字段（避免核心战斗引擎为单一来源增维）。已由 W5 端到端测试证明其确实削弱守方（`SubversionBattleIntegrationTests`）。
