...
> Accepted 2026-06-24（经跨系统审查 `design/gdd/gdd-cross-review-2026-06-24.md` 确认 N-#5 随机源契合 ADR-0004——已在 §Decision 1 补充显式声明；GDD_016 反全知锁/确定性/LLM 隔离与既有 ADR 无冲突）。
...
- **随机源契合 ADR-0004（不旁路）**：种子用于派生/定位 ADR-0004 的**注入式确定性随机流**，AI 仅在本 ADR 声明的决策检查点（动作选择、性格扰动）消费该流；**不**新建 `System.Random`/`UnityEngine.Random`/时间或帧依赖的旁路随机源。AI 的随机消费与战斗模拟同源，纳入同一状态哈希与重放契约（ADR-0004 §确定性、ADR-0005 §存档）。
